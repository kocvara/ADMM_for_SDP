function admm_dnn(sdpdata,maxiter)
%
% ADMM_DNN - Solves general linear conic problems with the cone of
% DOUBLE NONNEGATIVE matrices by the ADMM method as 
...
while err>eps
    ...
    for icon=1:ncon
        ...
        V{icon} = Vp{icon} - Z{icon} - mu.*X{icon};      
    ...
    %update Z
    for icon=1:ncon
        Up{icon} = C{icon} - [reshape(aaa{icon}*y,m(icon),m(icon))];
        U{icon} = Up{icon} - S{icon} - mu.*X{icon};      
        Z{icon} = max(U{icon},0);
        Z{icon}=sparse(.5.*(Z{icon}+Z{icon}'));
    end
...
%     end
    ...
    for icon=1:ncon
        UVSZ = [Up{icon}(:)-S{icon}(:)-Z{icon}(:)];
        dinfi =  sqrt(full(UVSZ'*UVSZ)); 
...
fprintf('%5d   %.8f   %.8f   %.8f   %.8f   %.6f   %.8f\n',count,pinf,dinf,dgap,mu,err,b'*y);
fprintf('-------------------------------------------------------------------------------------\n');
lambda_min = min(eig(X{1})); min_el = full(min(min(X{1})));
